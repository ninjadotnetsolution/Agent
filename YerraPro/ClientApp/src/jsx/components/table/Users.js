...
export default (props) => {
    ...
    useEffect(() => {
        ...
        setFilteredusers(users.filter(o => o.userName.includes(searchKey) || o.email.includes(searchKey)).slice(currentPage * sort, currentPage * sort + cnt));

    }, [users, currentPage])

    useEffect(() => {
        let paggination = Array(Math.ceil(users.filter(o => o.userName.includes(searchKey) || o.email.includes(searchKey)).length / sort))
            .fill()
            .map((_, i) => i + 1);
